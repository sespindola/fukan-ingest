ALTER TABLE telemetry_raw
    ADD COLUMN IF NOT EXISTS h3_res2 UInt64 MATERIALIZED h3ToParent(h3_cell, 2) AFTER h3_cell;

ALTER TABLE telemetry_raw
    ADD INDEX IF NOT EXISTS idx_h3_res2 h3_res2 TYPE bloom_filter GRANULARITY 3;

DROP VIEW IF EXISTS telemetry_latest_flat;
DROP VIEW IF EXISTS telemetry_latest_mv;

ALTER TABLE telemetry_latest
    ADD COLUMN IF NOT EXISTS h3_res2_state AggregateFunction(argMax, UInt64, DateTime64(3));
...
SELECT
    ...
    argMaxState(h3_res2, event_time)         AS h3_res2_state,
...
SELECT
    ...
    argMaxMerge(h3_res2_state)         AS h3_res2,
