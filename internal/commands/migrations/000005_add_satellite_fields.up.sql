ALTER TABLE telemetry_raw
    ADD COLUMN IF NOT EXISTS orbit_regime    LowCardinality(String)                      AFTER rate_of_turn,
    ADD COLUMN IF NOT EXISTS confidence      LowCardinality(String)                      AFTER orbit_regime,
    ADD COLUMN IF NOT EXISTS tle_epoch       DateTime64(3)           CODEC(DoubleDelta, LZ4) AFTER confidence,
    ADD COLUMN IF NOT EXISTS inclination     Float32                 CODEC(Gorilla, LZ4) AFTER tle_epoch,
    ADD COLUMN IF NOT EXISTS period_minutes  Float32                 CODEC(Gorilla, LZ4) AFTER inclination,
    ADD COLUMN IF NOT EXISTS apogee_km       Float32                 CODEC(Gorilla, LZ4) AFTER period_minutes,
    ADD COLUMN IF NOT EXISTS perigee_km      Float32                 CODEC(Gorilla, LZ4) AFTER apogee_km;

DROP VIEW IF EXISTS telemetry_latest_flat;
DROP VIEW IF EXISTS telemetry_latest_mv;

ALTER TABLE telemetry_latest
    ADD COLUMN IF NOT EXISTS orbit_regime_state   AggregateFunction(argMax, String, DateTime64(3)),
    ADD COLUMN IF NOT EXISTS confidence_state     AggregateFunction(argMax, String, DateTime64(3)),
    ADD COLUMN IF NOT EXISTS tle_epoch_state      AggregateFunction(argMax, DateTime64(3), DateTime64(3)),
    ADD COLUMN IF NOT EXISTS inclination_state    AggregateFunction(argMax, Float32, DateTime64(3)),
    ADD COLUMN IF NOT EXISTS period_minutes_state AggregateFunction(argMax, Float32, DateTime64(3)),
    ADD COLUMN IF NOT EXISTS apogee_km_state      AggregateFunction(argMax, Float32, DateTime64(3)),
    ADD COLUMN IF NOT EXISTS perigee_km_state     AggregateFunction(argMax, Float32, DateTime64(3));
